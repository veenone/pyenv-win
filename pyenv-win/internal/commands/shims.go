...
func (sc *ShimsCommand) Which(command string) (string, error) {
	...
	if err != nil {
		// Provide detailed error message about version configuration
		globalFile := filepath.Join(sc.cfg.PyenvHome, "version")
		if !utils.FileExists(globalFile) {
			return "", fmt.Errorf("pyenv: no Python version configured\nRun 'pyenv global <version>' to set a global version\nRun 'pyenv local <version>' to set a local version\nRun 'pyenv versions' to see installed versions")
		}
		return "", fmt.Errorf("pyenv: no Python version set: %w", err)
	}

	// Verify version exists before trying to resolve executable
	version := currentVersions[0]
	versionPath := filepath.Join(sc.cfg.VersionsDir, version)

	if !utils.DirExists(versionPath) {
		// Provide helpful error when version is configured but not installed
		versionSource := versionsCmd.findVersionSource()
		return "", fmt.Errorf("pyenv: version '%s' is configured (via %s) but not installed\nRun 'pyenv install %s' to install this version\nRun 'pyenv versions' to see installed versions", version, versionSource, version)
	}
	...
	// Enhanced error message when executable is not found
	return "", fmt.Errorf("pyenv: %s: command not found in Python version %s\n\nThe '%s' executable could not be found in:\n  %s\n  %s/Scripts/\n\nPossible solutions:\n  - Install the package containing '%s' using: python -m pip install <package>\n  - Run 'pyenv rehash' to refresh shims after installing packages\n  - Check if the command is available in other versions using: pyenv whence %s",
		command, version, command, versionPath, versionPath, command, command)
}
...
func (sc *ShimsCommand) createShim(exeName string) error {
	...
	// Get the path to the currently running pyenv executable
	pyenvExe, err := os.Executable()
	if err != nil {
		return fmt.Errorf("failed to get pyenv executable path: %w", err)
	}

	// Convert to absolute path and normalize
	pyenvExe, err = filepath.Abs(pyenvExe)
	if err != nil {
