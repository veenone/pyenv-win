...
import (
	...
	"github.com/pyenv-win/pyenv-win/internal/mirror"
)
...
func handleMirror(cfg *config.PyenvConfig, args []string) {
	if len(args) == 0 {
		showMirrorHelp()
		return
	}
	...
	// Handle init separately (doesn't need plugin)
	if subcommand == "init" {
		configPath := "mirror.json"
		if len(subargs) > 0 {
			configPath = subargs[0]
		}
		if err := mirror.InitConfig(configPath); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
		return
	...
	for i := 0; i < len(subargs); i++ {
		if subargs[i] == "-c" || subargs[i] == "--config" {
			...
			filteredArgs = append(filteredArgs, subargs[i])
		}
	}

	// Create plugin with config
	plugin, err := mirror.NewMirrorPlugin(cfg, configPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}
	...
	switch subcommand {
	case "all":
		if err := plugin.MirrorAll(); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	case "versions":
		if len(filteredArgs) == 0 {
			fmt.Println("Usage: pyenv mirror versions <version> [<version> ...]")
			os.Exit(1)
		}
		if err := plugin.MirrorVersions(filteredArgs); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	case "generate-mapping":
		if err := plugin.GenerateNexusMapping(); err != nil {
			fmt.Fprintf(os.Stderr, "Error: %v\n", err)
			os.Exit(1)
		}
	case "help", "--help":
		showMirrorHelp()
	default:
		fmt.Printf("pyenv: unknown mirror subcommand `%s'\n", subcommand)
		showMirrorHelp()
		os.Exit(1)
	}
}
...
func showMirrorHelp() {
	fmt.Println("Usage: pyenv mirror <subcommand> [options]")
	fmt.Println()
	fmt.Println("Subcommands:")
	fmt.Println("  init [path]                  Create a sample mirror configuration file (default: mirror.json)")
	fmt.Println("  all [-c|--config <file>]     Mirror all Python versions to Nexus repository")
	fmt.Println("  versions <version> [...]     Mirror specific Python versions to Nexus")
	fmt.Println("                               Options: -c|--config <file>")
	fmt.Println("  generate-mapping             Generate Nexus URL mapping file for offline usage")
	fmt.Println("  help                         Show this help message")
	fmt.Println()
	fmt.Println("Configuration:")
	fmt.Println("  The mirror feature uses a JSON configuration file. Create one with:")
	fmt.Println("    pyenv mirror init")
	fmt.Println()
	fmt.Println("  Default config file locations (checked in order):")
	fmt.Println("    1. mirror.json (current directory)")
	fmt.Println("    2. .mirror.json (current directory)")
	fmt.Println("    3. $PYENV/mirror.json")
	fmt.Println("    4. $PYENV_HOME/mirror.json")
	fmt.Println("    5. Environment variables (NEXUS_URL, NEXUS_USER, NEXUS_PASSWORD, etc.)")
	fmt.Println()
	fmt.Println("  You can also specify a custom config file:")
	fmt.Println("    pyenv mirror all --config /path/to/config.json")
	fmt.Println()
	fmt.Println("Build Requirements:")
	fmt.Println("  This feature requires compilation with the 'mirror' build tag:")
	fmt.Println("    go build -tags mirror -o pyenv.exe ./cmd/pyenv")
	fmt.Println()
	fmt.Println("Examples:")
	fmt.Println("  pyenv mirror init")
	fmt.Println("  pyenv mirror all")
	fmt.Println("  pyenv mirror all --config custom.json")
	fmt.Println("  pyenv mirror versions 3.12.0 3.11.5")
	fmt.Println("  pyenv mirror generate-mapping")
}
